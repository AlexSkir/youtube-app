...
exports.searchItemWrapper = searchItemWrapper;
...
function searchItemWrapper(data, i) {
  ...
  _onstart.itemsSection.appendChild(item);
  ...
  channelInfo.innerHTML = "# ".concat(data.snippet.channelTitle);
  infoBlock.appendChild(createdInfo);
  createdInfo.innerHTML = "created ".concat(data.snippet.publishedAt.split('T')[0]);
  infoBlock.appendChild(watchedInfo);
  watchedInfo.id = "data".concat(i);
...
function viewersCounter(viewCount, i) {
  document.getElementById("data".concat(i)).innerText = "".concat(viewCount, " views");
